...
public class CustomerGUI extends JFrame implements CustomerView
{
   ...
   private String[] seatTypes = new String[] {"All","Aisle", "Middle", "Window"};
   private JPasswordField fieldPass;
   private JDateChooser dateDeparture;
   private JDateChooser dateArrival;
   ...
   private void addComponentsToFrame()
   {
      ...
      labels[2].setBounds(810, 96, 61, 16);
      mainPanel.add(labels[2]);
      labels[3].setBounds(1020, 96, 77, 16);
      mainPanel.add(labels[3]);
      scrollPanes[0].setBounds(56, 95, 730, 539);
      mainPanel.add(scrollPanes[0]);
      Object rowData[][] = {};
      Object columnNames[] = { "Origin", "Destination", "Date", "Time", "Price"};
      ...
      comboBoxes[0].setBounds(860, 92, 140, 27);
      comboBoxes[0].setName("Origin");
      mainPanel.add(comboBoxes[0]);
      
      comboBoxes[1].setBounds(1100, 92, 140, 27);
      comboBoxes[1].setName("Destination");
      mainPanel.add(comboBoxes[1]);
      
      dateDeparture.setBounds(880, 146, 121, 26);
      dateDeparture.setName("dateDeparture");
      mainPanel.add(dateDeparture);
      dateArrival.setBounds(1104, 146, 121, 26);
      dateArrival.setName("dateArrival");
      mainPanel.add(dateArrival);
      labels[34].setBounds(810, 150, 77, 16);
      mainPanel.add(labels[34]);
      labels[35].setBounds(1045, 150, 61, 16);
      mainPanel.add(labels[35]);
      buttons[1].setBounds(960, 180, 117, 29);
      mainPanel.add(buttons[1]);
      buttons[2].setBounds(114, 637, 141, 29);
      mainPanel.add(buttons[2]);
      buttons[9].setBounds(530, 637, 141, 29);
      mainPanel.add(buttons[9]);
      ...
      labels[11].setBounds(256, 168, 400, 16); //168
      labels[11].setName("destination");
      bookPanel.add(labels[11]);
      

      labels[12].setBounds(280, 395, 200, 16);
      ...
      scrollPanes[2].setBounds(537, 102, 700, 273);
      ...
      buttons[10].setBounds(820, 400, 130, 29);
      viewProfilePanel.add(labels[22]);
      viewProfilePanel.add(buttons[10]);
      ...
      fieldPass.setBounds(623, 189, 178, 30);
      fieldPass.setColumns(10);
      fieldPass.setName("password");
      homePanel.add(fieldPass);
      ...
      fields[1].setBounds(629, 89, 180, 20);
      fields[1].setColumns(10);
      fields[1].setName("firstName");
      signUpPanel.add(fields[1]);
      fields[2].setColumns(10);
      fields[2].setBounds(629, 121, 180, 20);
      fields[2].setName("lastName");
      signUpPanel.add(fields[2]);
      fields[3].setColumns(10);
      fields[3].setBounds(629, 202, 180, 20);
      fields[3].setName("email");
      signUpPanel.add(fields[3]);
      fields[4].setColumns(10);
      fields[4].setBounds(629, 268, 180, 20);
      fields[4].setName("phone");
      signUpPanel.add(fields[4]);
      fields[5].setColumns(10);
      fields[5].setBounds(629, 304, 180, 20);
      fields[5].setName("usernameField");
      signUpPanel.add(fields[5]);
      fields[6].setColumns(10);
      fields[6].setBounds(629, 334, 180, 20);
      fields[6].setName("passwordField");
      signUpPanel.add(fields[6]);
      fields[7].setBounds(629, 234, 180, 26);
      fields[7].setColumns(10);
      fields[7].setName("passportNo");
   ...
   {
      setSize(1280, 750); // set frame size
   ...
   private void createComponents()
   {
      ...
      comboBoxes[2].setModel(new DefaultComboBoxModel<>(seatTypes));
      ...
      labels[34] = new JLabel("Departure:");
      labels[35] = new JLabel("Arrival:");
      buttons[9] = new JButton("Today's flights");
      buttons[10] = new JButton("Cancel ticket");
    }
   ...
   {
       String[] col = {"FlightID", "Origin", "Destination", "Date", "Time", "Price"};
       ...
       tables[0].getColumnModel().getColumn(0).setMaxWidth(0);
       tables[0].getColumnModel().getColumn(0).setMaxWidth(0);
       tables[0].getColumnModel().getColumn(1).setMinWidth(165);
       tables[0].getColumnModel().getColumn(2).setMinWidth(165);
       tables[0].getColumnModel().getColumn(3).setMinWidth(170);
   }
   
   private void loadTickets(String[] tickets)
   {
      String[] col = {"FlightID","TicketID", "Origin", "Destination", "Date", "Time", "Seat"}; 
      DefaultTableModel model = new DefaultTableModel(col, 0);
      ...
      tables[2].getColumnModel().getColumn(0).setMaxWidth(0);
      tables[2].getColumnModel().getColumn(0).setMaxWidth(0);
   }
   ...
   public String getTextFieldInput(String name)
   {
     ...
     if (output.isEmpty() && (fieldPass.getName().equals(name))) {
    	  output = passToString(fieldPass.getPassword());
      }
      return output;
   }

   
   private String passToString(char[] c) {
	   String s="";
   ...
   {
      int rowValue = getSelectedIndex("seatTable");
      return (String) tables[1].getValueAt(rowValue, 0);
   }
   ...
   public String getDates(String what)
   {
      ...
      try
      {
      if (dateDeparture.getName().equals(what))
      {
         localDate = dateDeparture.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
         output = localDate.getDayOfMonth() + "/" + localDate.getMonthValue() + "/" + localDate.getYear();
      }
      else if (dateArrival.getName().equals(what))
      {
         localDate = dateArrival.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
         output = localDate.getDayOfMonth() + "/" + localDate.getMonthValue() + "/" + localDate.getYear();
      }
      }
   ...
   {
      return (String) tables[0].getValueAt(tables[0].getSelectedRow(), 0);
   }

   @Override
   public String getTicket()
   {
      String output = (String) (tables[2].getValueAt(tables[2].getSelectedRow(), 0) + "<" + tables[2].getValueAt(tables[2].getSelectedRow(), 6));
      return output;
   }
